...
class ApiFutebolCalendarProviderFixed(ApiFutebolCalendarProvider):
    ...
    def _extract_detail_datetime(self, detail, home_name, away_name):
        """Busca a data/hora oficial no detalhe da partida, sem depender do resumo da fase."""
    ...
    def matches(self, date_from, date_to, competition=None):
        ...
        # A fase da API pode trazer a programação resumida/desatualizada.
        # Para as rodadas que podem entrar no período, consulta o detalhe de cada jogo
        # e usa a data/hora oficial do detalhe. Normalmente são apenas 10 jogos por rodada.
        detail_checked = 0
        detail_corrected = 0
        exact = {}
        for item, home, away, phase_dt, round_value in parsed:
            ...
            try:
                ...
                exact_dt, exact_item = self._extract_detail_datetime(detail, home['name'], away['name'])
                detail_checked += 1
                if exact_dt is not None:
                    exact[str(mid)] = exact_dt
                    if exact_dt.astimezone(MANAUS).date() != phase_dt.astimezone(MANAUS).date():
                        detail_corrected += 1
                        add_diagnostic('coleta', 'INFO', f'Série B: data corrigida {home["name"]} x {away["name"]}: fase={phase_dt.isoformat()} detalhe={exact_dt.isoformat()}', self.name, str(mid))
            except Exception as exc:
                add_diagnostic('coleta', 'WARNING', f'Série B: detalhe {mid} indisponível; usando data da fase: {exc}', self.name, str(mid))
        ...
        add_diagnostic('coleta', 'INFO', f'Série B DATA FIX: brutas={len(raw)}; válidas={len(parsed)}; inválidas={invalid}; rodadas candidatas={len(candidate_rounds)}; detalhes consultados={detail_checked}; datas corrigidas={detail_corrected}', self.name)

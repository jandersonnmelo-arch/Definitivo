...
def poisson(k,lam):
    if lam is None or not isinstance(lam,(int,float)) or lam<0 or k<0:return 0.0
    if lam==0:return 1.0 if k==0 else 0.0
    try:
        log_p=-lam+k*log(lam)-lgamma(k+1)
        if log_p<-745:return 0.0
        return exp(log_p)
    except (OverflowError,ValueError):return 0.0
def poisson_over(lam,line):
    if lam is None:return None
    k=int(line)+1;return round(max(0.0,1-sum(poisson(i,lam) for i in range(k)))*100,1)
def poisson_under(lam,line):
    if lam is None:return None
    k=int(line);return round(max(0.0,sum(poisson(i,lam) for i in range(k+1)))*100,1)
...
def _team_values(team_id,metric,before,limit=10):
    # Older records may have stored team tackles as `tackles` before the
    # team/player metric namespaces were separated. Read both spellings so
    # existing history remains usable while new data is canonicalized.
    metric_names=[metric]
    if metric=='effectivetackles':metric_names.append('tackles')
    placeholders=','.join('?' for _ in metric_names)
    c=connect();rows=c.execute(f'''SELECT m.id,m.start_time,s.value,s.source,s.metric
        FROM match_stats s JOIN matches m ON m.id=s.match_id
        WHERE s.team_id=? AND s.metric IN ({placeholders}) AND m.status='FINISHED' AND m.start_time < ?
        ORDER BY m.start_time DESC''',(team_id,*metric_names,before)).fetchall();c.close()
    by_match={}
    for r in rows:
        item=dict(r);old=by_match.get(item['id'])
        # Prefer the canonical spelling when both old and new records exist.
        rank=(0 if item.get('metric')==metric else 1,source_rank(item.get('source')))
        old_rank=(0 if old and old.get('metric')==metric else 1,source_rank(old.get('source')) if old else 99)
        if old is None or rank<old_rank:by_match[item['id']]=item
...
def _goals(team_id,before,limit=10):
    c=connect();rows=c.execute('''SELECT id,home_id,away_id,home_score,away_score FROM matches WHERE sport='Futebol' AND status='FINISHED' AND start_time < ? AND (home_id=? OR away_id=?) ORDER BY start_time DESC LIMIT ?''',(before,team_id,team_id,limit)).fetchall();c.close();scored=[];conceded=[]
    for r in rows:
...
def _projected_value(home,away,key):
    hv,av=home.get(key),away.get(key)
    return {'home':hv,'away':av,'total':round(hv+av,2) if isinstance(hv,(int,float)) and isinstance(av,(int,float)) else None,'sample_home':home.get(key+'_sample',0),'sample_away':away.get(key+'_sample',0)}
def build_pre_match_analysis(match,limit=10):
    ...
    markets={'gols':{'expected':total_xg,'total_expected':total_xg,'lines':{str(line):poisson_over(total_xg,line) for line in (0.5,1.5,2.5,3.5)} if total_xg is not None else {},'over':{str(line):poisson_over(total_xg,line) for line in (0.5,1.5,2.5,3.5)} if total_xg is not None else {},'under':{str(line):poisson_under(total_xg,line) for line in (1.5,2.5,3.5)} if total_xg is not None else {},'exact_total':exact_total_goals(total_xg,4)},'ambas_marcam':btts}

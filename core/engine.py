...
def outcome_probabilities(home_xg, away_xg):
    ...
    if not total:
        return {"home": None, "draw": None, "away": None}
    return {"home": round(home / total * 100, 1), "draw": round(draw / total * 100, 1), "away": round(away / total * 100, 1)}


def _norm_team_name(value):
    s = unicodedata.normalize("NFKD", str(value or "")).encode("ascii", "ignore").decode().lower()
    s = re.sub(r"\b(cr|ec|sc|se|ca|fc|cf|ac|club|football|futbol|de|da|do|dos|das)\b", " ", s)
...
def _team_values(team_id, metric, before, limit=10):
    ...
    connection = connect()
    try:
        target = connection.execute("SELECT name FROM teams WHERE id=?", (team_id,)).fetchone()
        target_name = target["name"] if target else ""
        rows = connection.execute(
            f"""SELECT m.id, m.start_time, s.value, s.source, s.metric, s.team_id, t.name AS stat_team_name
            FROM match_stats s
            JOIN matches m ON m.id = s.match_id
            LEFT JOIN teams t ON t.id = s.team_id
            WHERE s.metric IN ({placeholders})
    ...
    for row in rows:
        item = dict(row)
        exact_id = str(item.get("team_id")) == str(team_id)
        same_name = _same_team_name(target_name, item.get("stat_team_name"))
        if not exact_id and not same_name:
            continue
        old = by_match.get(item["id"])
        rank = (0 if exact_id else 1, 0 if item.get("metric") == metric else 1, source_rank(item.get("source")))
        old_rank = (0 if old and str(old.get("team_id")) == str(team_id) else 1, 0 if old and old.get("metric") == metric else 1, source_rank(old.get("source")) if old else 99)
        if old is None or rank < old_rank:
            by_match[item["id"]] = item

    selected = sorted(by_match.values(), key=lambda item: item.get("start_time") or "", reverse=True)[:limit]
...
def team_profile(team_id, before, limit=10):
    ...
    profile = {"sample": sample, "goals_for": scored, "goals_against": conceded}
    for key in METRICS:
        if key == "goals":
            continue
...
def _metric_market(home, away, key, lines):
    ...
    if home_value is None and away_value is None:
        return None
    values = [value for value in (home_value, away_value) if isinstance(value, (int, float))]
    if not values:
        return None
    ...
    return {"home": home_value, "away": away_value, "total_expected": round(total_expected, 2), "line": line, "over": over, "under": under, "over_probability": over, "under_probability": under, "lines": {str(line): over} if line is not None else {}, "under_lines": {str(line): under} if line is not None else {}}


def _projected_value(home, away, key):
    home_value = home.get(key)
    away_value = away.get(key)
    total = round(home_value + away_value, 2) if isinstance(home_value, (int, float)) and isinstance(away_value, (int, float)) else None
    return {"home": home_value, "away": away_value, "total": total, "sample_home": home.get(f"{key}_sample", 0), "sample_away": away.get(f"{key}_sample", 0)}


def build_pre_match_analysis(match, limit=10):
    home = team_profile(match["home_id"], match.get("start_time") or "", limit) if match.get("home_id") else {"sample": 0}
    away = team_profile(match["away_id"], match.get("start_time") or "", limit) if match.get("away_id") else {"sample": 0}
    home_attack = home.get("xg") if home.get("xg_sample", 0) > 0 else home.get("goals_for")
    away_attack = away.get("xg") if away.get("xg_sample", 0) > 0 else away.get("goals_for")
    home_xg = average([home_attack, away.get("goals_against")])
    away_xg = average([away_attack, home.get("goals_against")])
    xg_values = [value for value in (home_xg, away_xg) if isinstance(value, (int, float))]
    total_xg = round(sum(xg_values), 2) if xg_values else None
    probabilities = outcome_probabilities(home_xg, away_xg)
    both_score = None
    if home_xg is not None and away_xg is not None:
        both_score = round((1 - poisson(0, home_xg)) * (1 - poisson(0, away_xg)) * 100, 1)
    ...
    markets = {"gols": {"expected": total_xg, "total_expected": total_xg, "line": goal_line, "lines": {str(goal_line): goal_over} if goal_line is not None else {}, "under_lines": {str(goal_line): goal_under} if goal_line is not None else {}, "over": {str(goal_line): goal_over} if goal_line is not None else {}, "under": {str(goal_line): goal_under} if goal_line is not None else {}, "over_probability": goal_over, "under_probability": goal_under, "exact_total": exact_total_goals(total_xg, 4), "ambas_marcam": both_score}}
    ...
    projections = {"goals": {"home": home_xg, "away": away_xg, "total": total_xg, "sample_home": home.get("sample", 0), "sample_away": away.get("sample", 0)}}
    for key in projection_keys:
        projections[key] = _projected_value(home, away, key)
    coverage = {key: {"home": home.get(f"{key}_sample", 0), "away": away.get(f"{key}_sample", 0)} for key in METRICS}
    return {"home": home, "away": away, "xg_home": home_xg, "xg_away": away_xg, "probabilities": probabilities, "coverage": coverage, "sample_home": home.get("sample", 0), "sample_away": away.get("sample", 0), "markets": markets, "btts": both_score, "projections": projections}

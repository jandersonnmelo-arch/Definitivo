...
def _team_values(team_id, metric, before, limit=10):
    c = connect()
    rows = c.execute('''SELECT m.id,m.start_time,s.value,s.source
        FROM match_stats s JOIN matches m ON m.id=s.match_id
        WHERE s.team_id=? AND s.metric=? AND m.status='FINISHED' AND m.start_time < ?
        ORDER BY m.start_time DESC''', (team_id, metric, before)).fetchall()
    c.close()
    by_match = {}
    for r in rows:
        item = dict(r)
        old = by_match.get(item['id'])
        if old is None or source_rank(item.get('source')) < source_rank(old.get('source')):
...
def _goals(team_id, before, limit=10):
    c = connect()
    rows = c.execute('''SELECT id,home_id,away_id,home_score,away_score
        FROM matches WHERE sport='Futebol' AND status='FINISHED'
        AND start_time < ? AND (home_id=? OR away_id=?)
        ORDER BY start_time DESC LIMIT ?''', (before, team_id, team_id, limit)).fetchall()
    c.close()
    scored, conceded = [], []
    for r in rows:
...
def team_profile(team_id, before, limit=10):
    scored, conceded, n = _goals(team_id, before, limit)
    out = {'sample': n, 'goals_for': scored, 'goals_against': conceded}
    for key in METRICS:
        if key == 'goals':
            continue
        vals = _team_values(team_id, key, before, limit)
        out[key] = average(vals)
        out[key + '_sample'] = len(vals)
    out['xg'] = out.get('expected_goals')
    out['xg_sample'] = out.get('expected_goals_sample', 0)
    return out
...
def _projected_value(home, away, key):
    ...
    return {
        'home': hv,
        'away': av,
        'total': round(hv + av, 2) if isinstance(hv, (int, float)) and isinstance(av, (int, float)) else None,
        'sample_home': home.get(key + '_sample', 0),
        'sample_away': away.get(key + '_sample', 0),
    }


def build_pre_match_analysis(match, limit=10):
    h = team_profile(match['home_id'], match.get('start_time') or '', limit) if match.get('home_id') else {'sample': 0}
    a = team_profile(match['away_id'], match.get('start_time') or '', limit) if match.get('away_id') else {'sample': 0}
    ...
    hxg = average([h_attack, a.get('goals_against')])
    axg = average([a_attack, h.get('goals_against')])
    total_xg = round(sum(x for x in (hxg, axg) if isinstance(x, (int, float))), 2) if hxg is not None or axg is not None else None
    ...
    markets = {
        'gols': {
            ...
            'lines': {str(line): poisson_over(total_xg, line) for line in (0.5, 1.5, 2.5, 3.5)} if total_xg is not None else {},
            'over': {str(line): poisson_over(total_xg, line) for line in (0.5, 1.5, 2.5, 3.5)} if total_xg is not None else {},
            'under': {str(line): poisson_under(total_xg, line) for line in (1.5, 2.5, 3.5)} if total_xg is not None else {},
            'exact_total': exact_total_goals(total_xg, 4),
        },
        'ambas_marcam': btts,
    }
    for name, (key, lines) in market_specs.items():
        markets[name] = _metric_market(h, a, key, lines)
    ...
    for key in projection_keys:
        projections[key] = _projected_value(h, a, key)

...
def position_label(position):
 ...
 raw=str(position or '').strip();key=raw.upper().replace(' ','_').replace('-','-')
...
def normalize_status(status,completed=None):
 ...
 return 'SCHEDULED'
def normalize_metric(name):return MATCH_METRIC_ALIASES.get(_key(name).lower(),_key(name).lower())
def average(values):

...
def connect():
    """Open a SQLite connection configured for concurrent Streamlit writes.

    WAL is intentionally NOT switched on for every connection. Reissuing
    PRAGMA journal_mode=WAL on every open can itself require an exclusive
    lock and was the source of intermittent OperationalError during history
    reconciliation. WAL is initialized once by init_db().
    """
...
def _norm(x):
    s = (
        unicodedata.normalize("NFKD", str(x or ""))
        .encode("ascii", "ignore")
        .decode()
        .lower()
    )
...
def canonical_id(m):
    return hashlib.sha1(
        "|".join(
            [
                _norm(m.get("sport", "Futebol")),
                _norm(m.get("home_name")),
                _norm(m.get("away_name")),
                str(m.get("start_time") or "")[:16],
            ]
        ).encode()
    ).hexdigest()[:20]
...
def init_db():
    with _DB_WRITE_LOCK:
        c = connect()
        try:
            # Initialize WAL only during database initialization, never on
            # every connection. busy_timeout above handles concurrent access.
            try:
                c.execute("PRAGMA journal_mode=WAL")
            except sqlite3.OperationalError:
                # The database can already be in WAL mode or temporarily busy.
                # Do not make application startup fail solely because of this.
                pass
            ...
            c.executescript(
                ...
                CREATE TABLE IF NOT EXISTS match_sources(
                    ...
                    UNIQUE(source,provider_match_id)
                );
            ...
            c.execute(
                "UPDATE players SET source=COALESCE(source,'LEGACY'),"
                "provider_player_id=COALESCE(provider_player_id,CAST(id AS TEXT)),"
                "updated_at=COALESCE(updated_at,?)",
                (now_iso(),),
            )
            c.execute(
                "INSERT OR REPLACE INTO schema_meta(key,value) VALUES('schema','14')"
            )
            c.commit()
        finally:
            c.close()
...
def _team_id(c, sport, name, source=None, provider_id=None):
    ...
    c.execute(
        """INSERT INTO teams(id,sport,name,normalized_name,updated_at)
           VALUES(?,?,?,?,?)
           ON CONFLICT(id) DO UPDATE SET
             name=excluded.name,
             updated_at=excluded.updated_at""",
        (cid, sport, name, _norm(name), now_iso()),
    )
    if source and provider_id is not None:
        c.execute(
            """INSERT INTO team_sources(team_id,source,provider_team_id,updated_at)
               VALUES(?,?,?,?)
               ON CONFLICT(team_id,source) DO UPDATE SET
                 provider_team_id=excluded.provider_team_id,
                 updated_at=excluded.updated_at""",
            (cid, source, str(provider_id), now_iso()),
        )
    return cid
...
def record_api_usage(provider, remaining_day=None, remaining_minute=None):
    with _DB_WRITE_LOCK:
        ...
        try:
            ...
            c.execute(
                "INSERT INTO api_call_log(provider,created_at) VALUES(?,?)",
                (provider, now),
            )
            c.execute(
                """INSERT INTO api_usage(
                       provider,day_utc,calls,last_call,remaining_day,remaining_minute
                   ) VALUES(?,?,?,?,?,?)
                   ON CONFLICT(provider,day_utc) DO UPDATE SET
                     calls=calls+1,
                     last_call=excluded.last_call,
                     remaining_day=COALESCE(excluded.remaining_day,api_usage.remaining_day),
                     remaining_minute=COALESCE(excluded.remaining_minute,api_usage.remaining_minute)""",
                (provider, day, 1, now, remaining_day, remaining_minute),
            )
            c.commit()
        finally:
            c.close()
...
def usage_today(provider):
    ...
    try:
        day = datetime.now(timezone.utc).date().isoformat()
        r = c.execute(
            "SELECT * FROM api_usage WHERE provider=? AND day_utc=?",
            (provider, day),
        ).fetchone()
...
def calls_last_minute(provider):
    ...
    try:
        ...
        n = c.execute(
            "SELECT COUNT(*) FROM api_call_log WHERE provider=? AND created_at>=?",
            (provider, cutoff),
        ).fetchone()[0]
        return int(n)
    finally:
...
def upsert_match(m):
    with _DB_WRITE_LOCK:
        ...
        for attempt in range(3):
            c = connect()
            try:
                mid = m.get("id") or canonical_id(m)
                m["id"] = mid
                home_cid = _team_id(
                    c,
                    m.get("sport", "Futebol"),
                ...
                m["away_id"] = away_cid
                old = c.execute(
                    "SELECT source FROM matches WHERE id=?", (mid,)
                ).fetchone()
                source = m.get("source", "unknown")
                if old and old["source"] and old["source"] != source:
                    source = "multi"

                c.execute(
                    """INSERT INTO matches(
                         id,sport,competition,season,start_time,status,minute,
                         home_id,home_name,home_short,away_id,away_name,away_short,
                         home_score,away_score,source,provider_match_id,updated_at
                       ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                       ON CONFLICT(id) DO UPDATE SET
                         ...
                         home_short=COALESCE(excluded.home_short,matches.home_short),
                         away_id=excluded.away_id,
                         away_name=excluded.away_name,
                         away_short=COALESCE(excluded.away_short,matches.away_short),
                    ...
                    (
                        ...
                        m.get("away_score"),
                        source,
                        str(m.get("provider_match_id", mid)),
                        now_iso(),
                    ),
                )
                c.execute(
                    """INSERT INTO match_sources(
                         match_id,source,provider_match_id,updated_at
                       ) VALUES(?,?,?,?)
...
def get_provider_id(match_id, source):
    ...
    try:
        r = c.execute(
            "SELECT provider_match_id FROM match_sources WHERE match_id=? AND source=?",
            (match_id, source),
        ).fetchone()
        return r["provider_match_id"] if r else None
    finally:
        c.close()
...
def get_team_provider_id(team_id, source):
    ...
    try:
        r = c.execute(
            "SELECT provider_team_id FROM team_sources WHERE team_id=? AND source=?",
            (team_id, source),
        ).fetchone()
        return r["provider_team_id"] if r else None
    finally:
        c.close()
...
def _canonical_stat_team(c, match_id, source, provider_team_id, team_name):
    r = c.execute(
        "SELECT team_id FROM team_sources WHERE source=? AND provider_team_id=?",
        (source, str(provider_team_id)),
    ).fetchone()
    if r:
        return r["team_id"]
    ...
    return _team_id(
        c,
        m["sport"] if m else "Futebol",
        team_name or str(provider_team_id),
        source,
        provider_team_id,
    )
...
def upsert_match_stats(match_id, rows):
    ...
    with _DB_WRITE_LOCK:
        ...
        try:
            for r in rows:
                if r.get("team_id") is None or r.get("value") is None:
                    continue
                tid = _canonical_stat_team(
                    c,
                    match_id,
                    r["source"],
                    r["team_id"],
                ...
                c.execute(
                    """INSERT INTO match_stats(
                         match_id,team_id,metric,value,source,observed_at
                       ) VALUES(?,?,?,?,?,?)
                       ON CONFLICT(match_id,team_id,metric,source) DO UPDATE SET
    ...
    with _DB_WRITE_LOCK:
        ...
        try:
            for p in players:
                ...
                cid = canonical_player_id(src, p["id"])
                team = p.get("team_id")
                team_cid = (
                    _canonical_stat_team(
                        c,
                        p.get("match_id", ""),
                        src,
                        team,
                    ...
                    if team is not None and p.get("match_id")
                    else team
                )
                c.execute(
                    """INSERT INTO players(
                         id,team_id,name,position,source,provider_player_id,updated_at
                       ) VALUES(?,?,?,?,?,?,?)
                       ON CONFLICT(id) DO UPDATE SET
    ...
    with _DB_WRITE_LOCK:
        ...
        try:
            for r in rows:
                ...
                cid = canonical_player_id(src, r["player_id"])
                c.execute(
                    """INSERT INTO player_stats(
                         match_id,player_id,metric,value,source,observed_at
                       ) VALUES(?,?,?,?,?,?)
                       ON CONFLICT(match_id,player_id,metric,source) DO UPDATE SET
                         value=excluded.value,
                         observed_at=excluded.observed_at""",
                    (
                        match_id,
        ...
        try:
            c.execute(
                """INSERT INTO diagnostics(
                     match_id,source,stage,status,message,created_at
                   ) VALUES(?,?,?,?,?,?)""",
                (match_id, source, stage, status, message, now_iso()),
            )
            c.commit()
        finally:
            c.close()
...
def get_stats(mid):
    ...
    try:
        rows = c.execute(
            "SELECT * FROM match_stats WHERE match_id=? ORDER BY metric,team_id,source",
            (mid,),
        ).fetchall()
...
def get_players(mid):
    ...
    try:
        rows = c.execute(
            """SELECT p.id,p.team_id,p.name,p.position,
                      ps.metric,ps.value,ps.source
               FROM players p
               JOIN player_stats ps ON ps.player_id=p.id
               WHERE ps.match_id=?
               ORDER BY p.team_id,p.name,ps.metric""",
            (mid,),
        ).fetchall()
        return [dict(x) for x in rows]
    finally:
...
def team_history(team_id, before_iso=None, limit=10):
    ...
    try:
        sql = (
            "SELECT * FROM matches WHERE sport='Futebol' "
            "AND status='FINISHED' AND (home_id=? OR away_id=?)"
        )
        p = [team_id, team_id]
        if before_iso:
            sql += " AND start_time < ?"
...
def metric_history(team_id, metric, before_iso=None, limit=10):
    ...
    try:
        sql = (
            "SELECT m.start_time,s.value FROM match_stats s "
            "JOIN matches m ON m.id=s.match_id "
            "WHERE s.team_id=? AND s.metric=? AND m.status='FINISHED'"
        )
        p = [team_id, metric]
        if before_iso:
...
def player_history(player_id, before_iso=None, limit=20):
    ...
    try:
        sql = (
            "SELECT ps.match_id,ps.metric,ps.value,ps.source,m.start_time,"
            "m.home_name,m.away_name FROM player_stats ps "
            "JOIN matches m ON m.id=ps.match_id "
            "WHERE ps.player_id=? AND m.status='FINISHED'"
        )
        p = [player_id]
        if before_iso:
            sql += " AND m.start_time < ?"
...
def player_history_summary(team_id, before_iso=None, limit=20):
    ...
    try:
        sql = """
            ...
            JOIN player_stats ps ON ps.player_id=p.id
            JOIN matches m ON m.id=ps.match_id
            WHERE p.team_id=? AND m.status='FINISHED'
        """
        p = [team_id]
        if before_iso:
            sql += " AND m.start_time < ?"
            p.append(before_iso)
        sql += " GROUP BY p.id,p.name,p.position ORDER BY matches DESC,p.name LIMIT ?"
        p.append(limit)
        rows = c.execute(sql, p).fetchall()
        return [dict(x) for x in rows]
    finally:
...
def history_coverage(team_id, before_iso=None):
    ...
    try:
        sql = (
            "SELECT COUNT(*) FROM matches WHERE sport='Futebol' "
            "AND status='FINISHED' AND (home_id=? OR away_id=?)"
        )
        p = [team_id, team_id]
        if before_iso:

...
TEAM_ALIASES={'bayer leverkusen':'bayer 04 leverkusen','bayer 04':'bayer 04 leverkusen','bayer 04 leverkusen':'bayer 04 leverkusen','bayern munich':'bayern munchen','bayern munchen':'bayern munchen','bayern munchen fc':'bayern munchen','fc bayern munchen':'bayern munchen','atletico mineiro':'atletico mineiro','atletico mg':'atletico mineiro','ca mineiro':'atletico mineiro','clube atletico mineiro':'atletico mineiro','cam':'atletico mineiro','atletico paranaense':'athletico paranaense','athletico paranaense':'athletico paranaense','athletico pr':'athletico paranaense','cap':'athletico paranaense','flamengo':'flamengo','flamengo rj':'flamengo','cr flamengo':'flamengo','palmeiras':'palmeiras','se palmeiras':'palmeiras','sao paulo':'sao paulo','sao paulo fc':'sao paulo','corinthians':'corinthians','corinthians paulista':'corinthians','sport corinthians paulista':'corinthians','sport club corinthians paulista':'corinthians','santos':'santos','santos fc':'santos','gremio':'gremio','gremio fbpa':'gremio','internacional':'internacional','sport club internacional':'internacional','cruzeiro':'cruzeiro','cruzeiro esporte clube':'cruzeiro','botafogo':'botafogo','botafogo fr':'botafogo','fluminense':'fluminense','vasco da gama':'vasco da gama','vasco':'vasco da gama','bahia':'bahia','ec bahia':'bahia','vitoria':'vitoria','fortaleza':'fortaleza','ceara':'ceara','sport recife':'sport recife','sport':'sport recife','bragantino':'red bull bragantino','red bull bragantino':'red bull bragantino','red bull brasil':'red bull bragantino','lille':'lille','lille osc':'lille','paris saint germain':'paris saint germain','paris saint germain fc':'paris saint germain','psg':'paris saint germain','racing santander':'real racing club de santander','racing club de santander':'real racing club de santander','real racing':'real racing club de santander','real racing club de santander':'real racing club de santander','real racing de santander':'real racing club de santander','sporting clube de portugal':'sporting portugal','clube de portugal':'sporting portugal','sporting portugal':'sporting portugal','sporting cp':'sporting portugal','sporting lisbon':'sporting portugal'}
def _norm(value):
...
def reconcile_database(force=False):
    ...
    for old,new in team_map.items():
        ...
        c.execute('DELETE FROM match_stats WHERE team_id=?',(old,));c.execute('UPDATE players SET team_id=? WHERE team_id=?',(new,old));c.execute('UPDATE player_stats SET team_id=? WHERE team_id=?',(new,old))
    ...
    for _,rows in groups.items():
        ...
        for dup in rows[1:]:
            old_id=dup['id'];stat_rows=c.execute('SELECT match_id,team_id,metric,value,source,observed_at FROM player_stats WHERE player_id=?',(old_id,)).fetchall()
            for s in stat_rows:
                try:c.execute('INSERT INTO player_stats(match_id,player_id,team_id,metric,value,source,observed_at) VALUES(?,?,?,?,?,?,?)',(s['match_id'],canonical_id,s['team_id'],s['metric'],s['value'],s['source'],s['observed_at']));stats_migrated+=1
                except Exception:pass
    ...
    for d in dup_rows:
        ...
        for r in ranked[1:]:c.execute('DELETE FROM player_stats WHERE match_id=? AND player_id=? AND metric=? AND source=?',(d['match_id'],d['player_id'],d['metric'],r['source']));stats_deduped+=1
    ...
    try:serie_b=reconcile_serie_b_matches()
    except Exception:serie_b={'matches_merged':0,'sources_moved':0,'stats_moved':0,'player_stats_moved':0}

...
def _collect_team_history_from_espn(team_name,before_iso,days=120,serie_b=False):
    """Coleta histórico usando o mesmo calendário geral da aplicação.

    Para a Série B usamos o ESPNCalendarProvider, que consulta o slug bra.2
    com um intervalo de datas em uma única requisição, em vez de consultar
    /all dia a dia. Dados Futebol permanece exclusivo do enriquecimento.
    """
    provider=ESPNCalendarProvider()
    before=_parse_start(before_iso) or datetime.now(timezone.utc)
    start=(before-timedelta(days=days)).date().isoformat();end=before.date().isoformat()
    competition='🇧🇷 Campeonato Brasileiro Série B' if serie_b else None
    rows=[]
    try:
        for m in provider.matches(start,end,competition):
        ...
        add_diagnostic('historico','OK',f'{provider.name} calendário: {len(rows)} partidas históricas coletadas para {team_name}',provider.name)
    except Exception as e:
        add_diagnostic('historico','ERROR',f'{provider.name} calendário histórico: {e}',provider.name)
    return rows
...
def build_history_for_match(match,matches_per_team=HISTORY_MATCHES_PER_TEAM,days=HISTORY_DAYS):
    ...
    # Primeiro aproveita o que já foi coletado pelo calendário.
    # Para Série B, se faltar histórico, consulta o slug bra.2 uma única vez
    # para as duas equipes. Dados Futebol não participa desta etapa.
    if serie_b:
        missing_teams=[]
        ...
        if missing_teams:
            provider=ESPNCalendarProvider();before=_parse_start(before_iso) or datetime.now(timezone.utc);start=(before-timedelta(days=days)).date().isoformat();end=before.date().isoformat()
            try:
                rows=provider.matches(start,end,'🇧🇷 Campeonato Brasileiro Série B')
                matched=0
                for m in rows:
                    if _parse_start(m.get('start_time')) and _parse_start(m.get('start_time'))>before:continue
                    if any(_same_team(name,m.get('home_name')) or _same_team(name,m.get('away_name')) for name in missing_teams):
                        upsert_match(m);matched+=1
                add_diagnostic('historico','OK',f'ESPN calendário Série B: {matched} partidas históricas persistidas para {len(missing_teams)} equipes em uma única coleta',provider.name,match.get('id'))
            except Exception as e:add_diagnostic('historico','ERROR',f'ESPN calendário Série B histórico: {e}',provider.name,match.get('id'))
    else:
        for team_id in team_ids:
            team_name=match['home_name'] if team_id==match.get('home_id') else match['away_name']
            if history_coverage(team_id,before_iso)<matches_per_team:
            ...
            if len(hist)<matches_per_team:
                _collect_team_history_from_espn(team_name,before_iso,120,False)

...
MIN_PUSH_SECONDS=int(os.getenv("DEFINITIVO_REMOTE_PUSH_MIN_SECONDS","20"));_LAST_PUSH=0.0
TABLES=["schema_meta","teams","team_sources","matches","match_sources","match_stats","players","player_stats","diagnostics","api_usage","api_call_log","ai_match_samples","ai_player_samples","ai_predictions","ai_model_runs","ai_dataset_v1"]
def _token():
...
def snapshot_payload():
    ...
    c=connect()
    try:
        existing={r["name"] for r in c.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()};out={"format":"definitivo-db-snapshot-v4","created_at":datetime.now(timezone.utc).isoformat(),"tables":{}}
        for table in TABLES:
            if table in existing:out["tables"][table]=[dict(r) for r in c.execute(f"SELECT * FROM {table}").fetchall()]
        return out
    finally:c.close()
def push(force=False):
    ...
    now=time.monotonic()
    if not force and _LAST_PUSH and now-_LAST_PUSH<MIN_PUSH_SECONDS:return {"pushed":False,"reason":"debounced"}
    content=json.dumps(snapshot_payload(),ensure_ascii=False,indent=2,default=str);headers={"Authorization":f"Bearer {token}","Accept":"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28"}
    try:
        r=requests.get(_url(),params={"ref":REMOTE_BRANCH},headers=headers,timeout=15);sha=r.json().get("sha") if r.status_code==200 else None
        if r.status_code not in (200,404):return {"pushed":False,"reason":f"github_read_{r.status_code}"}
        body={"message":"Persist database and AI snapshot","content":base64.b64encode(content.encode()).decode(),"branch":REMOTE_BRANCH}
        if sha:body["sha"]=sha
        r=requests.put(_url(),headers=headers,json=body,timeout=30)
        if r.status_code in (200,201):_LAST_PUSH=now;return {"pushed":True,"reason":"ok"}
        return {"pushed":False,"reason":f"github_write_{r.status_code}"}
    except Exception as exc:return {"pushed":False,"reason":type(exc).__name__}
def pull():
    try:

...
with st.sidebar:
    ...
    date_from, date_to = today, today + timedelta(days=days)
    ...
    if st.button('🔄 Atualizar jogos', use_container_width=True, type='primary'):
        with st.spinner('Coletando partidas...'):
            rows = collect(date_from.isoformat(), date_to.isoformat(), competitions=selected_labels(optional_selected))
...
for m in window_matches:
    score_h = m.get('home_score') if m.get('home_score') is not None else '·'
    score_a = m.get('away_score') if m.get('away_score') is not None else '·'
    if st.button(f'{m["home_name"]}  {score_h} × {score_a}  {m["away_name"]}', key=f'm_{m["id"]}', use_container_width=True):
...
if selected:
    ...
    if m:
        st.divider()
        st.subheader(f'📊 {m["home_name"]} × {m["away_name"]}')
        ...
        analysis = build_pre_match_analysis(m) if m.get('status') == 'SCHEDULED' else None

        st.markdown('### 📈 Estatísticas da partida')
        if m.get('status') == 'SCHEDULED':
            projections = (analysis or {}).get('projections', {})
            if projections:
                rows = []
                for canonical in MATCH_DISPLAY_ORDER:
                    pr = projections.get(canonical, {})
                    rows.append({'Indicador': MATCH_DISPLAY_LABELS[canonical], 'Casa': format_metric_value(canonical, pr.get('home')), 'Fora': format_metric_value(canonical, pr.get('away')), 'Total': format_metric_value(canonical, pr.get('total'))})
                st.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)
                st.caption('Valores acima são projeções pré-jogo baseadas no histórico persistido; não são estatísticas já ocorridas.')
            else:
                st.info('Ainda não há histórico suficiente para projetar as estatísticas.')
        elif stats or m.get('home_score') is not None or m.get('away_score') is not None:
            rows = []
            for canonical in MATCH_DISPLAY_ORDER:
                if canonical == 'goals':
                    home, away = m.get('home_score'), m.get('away_score')
                else:
                    hr = choose_preferred_stat([r for r in stats if r.get('team_id') == m['home_id'] and canonical_match_metric(r.get('metric')) == canonical])
                    ar = choose_preferred_stat([r for r in stats if r.get('team_id') == m['away_id'] and canonical_match_metric(r.get('metric')) == canonical])
                    home = hr.get('value') if hr else None
                    away = ar.get('value') if ar else None
        ...
        st.markdown('### 📚 Histórico das equipes')
        c1, c2 = st.columns(2)
        c1.metric(m['home_name'], len(home_hist))
        c2.metric(m['away_name'], len(away_hist))
        for tname, hist in ((m['home_name'], home_hist), (m['away_name'], away_hist)):
            with st.expander(f'⚽ Últimos {len(hist)} jogos — {tname}', expanded=False):
                if hist:
                    st.dataframe(pd.DataFrame([{'Data': manaos_time(x.get('start_time')), 'Competição': canonical_competition_label(x.get('competition')), 'Jogo': f'{x["home_name"]} {x.get("home_score", "-")} × {x.get("away_score", "-")} {x["away_name"]}'} for x in hist]), hide_index=True, use_container_width=True)
                else:
                    st.info('Nenhum jogo histórico persistido.')
        ...
        if m.get('status') == 'SCHEDULED':
            if st.button('📚 Atualizar histórico + estatísticas + jogadores', key=f'h_{selected}', use_container_width=True, type='primary'):
                ...
                st.success(f'Histórico: {result["home_matches"]} + {result["away_matches"]} jogos • {result["stats_records"]} estatísticas • {result["player_records"]} registros individuais • {len(result.get("current_players", []))} jogadores da partida.')
                st.rerun()
        elif len(home_hist) < 10 or len(away_hist) < 10:
            if st.button('📚 Completar histórico', key=f'h_{selected}', use_container_width=True):
                result = build_history_for_match(m)
                st.success(f'Histórico atualizado: {result["stats_records"]} estatísticas e {result["player_records"]} individuais.')
                st.rerun()
        ...
        if m.get('status') == 'SCHEDULED':
            ...
            if p['home'] is None:
                st.info('Ainda não há histórico suficiente. Atualize o histórico acima.')
            else:
                c1, c2, c3 = st.columns(3)
                c1.metric('Casa', f'{p["home"]}%')
                c2.metric('Empate', f'{p["draw"]}%')
                c3.metric('Fora', f'{p["away"]}%')
            ...
            metric_rows = []
            base_keys = [('goals_for', 'Gols feitos'), ('goals_against', 'Gols sofridos'), ('shots', 'Finalizações totais'), ('shots_on_target', 'Finalizações no alvo'), ('woodwork', 'Finalizações na trave'), ('effectivetackles', 'Desarmes efetivos'), ('corners', 'Escanteios'), ('fouls', 'Faltas'), ('saves', 'Defesas do goleiro'), ('player_throws', 'Laterais'), ('yellow_cards', 'Cartões amarelos'), ('red_cards', 'Cartões vermelhos'), ('offsides', 'Impedimentos'), ('goal_kicks', 'Tiros de meta'), ('passes_completed', 'Passes certos')]
            for key, label in base_keys:
                hv, av = analysis['home'].get(key), analysis['away'].get(key)
            ...
            if metric_rows:
                ...
                st.warning('Há jogos históricos, mas as métricas detalhadas ainda não foram persistidas.')

            st.markdown('### 🎯 Palpites estatísticos')
            market_labels = {'gols': 'Gols', 'finalizacoes': 'Finalizações totais', 'finalizacoes_no_alvo': 'Finalizações no alvo', 'finalizacoes_na_trave': 'Finalizações na trave', 'desarmes_efetivos': 'Desarmes efetivos', 'escanteios': 'Escanteios', 'faltas': 'Faltas', 'defesas': 'Defesas do goleiro', 'laterais': 'Laterais', 'cartoes_amarelos': 'Cartões amarelos', 'cartoes_vermelhos': 'Cartões vermelhos', 'impedimentos': 'Impedimentos', 'tiros_de_meta': 'Tiros de meta', 'passes_certos': 'Passes certos'}
            rows = []
            for key, label in market_labels.items():
                obj = analysis['markets'].get(key) or {}
                expected = obj.get('total_expected')
                lines = obj.get('lines') or {}
                if expected is not None:
                    best = max(lines.items(), key=lambda item: item[1]) if lines else (None, None)
                    rows.append({'Mercado': label, 'Projeção total': expected, 'Linha': f'Over {best[0]}' if best[0] else '—', 'Probabilidade': f'{best[1]}%' if best[1] is not None else '—'})
            if rows:
                st.dataframe(pd.DataFrame(rows), hide_index=True, use_container_width=True)
            else:
                st.info('Sem amostra estatística suficiente para mercados adicionais.')
            if analysis['markets'].get('ambas_marcam') is not None:
                st.metric('🤝 Ambas marcam — SIM', f'{analysis["markets"]["ambas_marcam"]}%')
            exact = (analysis['markets'].get('gols') or {}).get('exact_total') or {}
            if exact:
                st.markdown('#### ⚽ Distribuição de gols')
                st.dataframe(pd.DataFrame([{'Total de gols': k, 'Probabilidade': f'{v}%'} for k, v in exact.items()]), hide_index=True, use_container_width=True)
            st.caption('Probabilidades são calculadas por Poisson sobre médias históricas persistidas. Ausência de dado não vira zero.')

            for tid, tname in ((m.get('home_id'), m.get('home_name')), (m.get('away_id'), m.get('away_name'))):
                if not tid:
                    continue
                summary = player_history_summary(tid, before, 20)
                st.markdown(f'#### 👥 Histórico individual — {tname}')
                if summary:
                    st.dataframe(pd.DataFrame([{'Jogador': x['name'], 'Jogos': x['matches'], 'Gols': format_metric_value('goals', x['goals']), 'Assistências': format_metric_value('assists', x['assists']), 'Finalizações': format_metric_value('shots', x['shots']), 'No alvo': format_metric_value('shots_on_target', x['shots_on_target']), 'Passes certos': format_metric_value('passes_completed', x['passes_completed']), 'Desarmes': format_metric_value('tackles', x['tackles'])} for x in summary]), hide_index=True, use_container_width=True)
                else:
                    st.info('Nenhum dado individual histórico persistido para esta equipe.')

        players = get_players(selected)
        unique = {row['id']: row for row in players}
        st.subheader(f'👥 Jogadores da partida • {len(unique)}')
        if unique:
            teams = {}
            for row in players:
                teams.setdefault(row.get('team_id'), []).append(row)
            ordered_team_ids = [m.get('home_id'), m.get('away_id')] + [x for x in teams if x not in (m.get('home_id'), m.get('away_id'))]
            for tid in ordered_team_ids:
                plist = teams.get(tid, [])
                if not plist:
                    continue
                team_name = m['home_name'] if tid == m.get('home_id') else m['away_name'] if tid == m.get('away_id') else 'Equipe'
                ...
                for pid in sorted({p['id'] for p in plist}, key=lambda x: next((p['name'] for p in plist if p['id'] == x), '')):
                    ...
                    with st.expander(f'👤 {p0["name"]} • {position_label(p0.get("position"))}', expanded=False):
                        ...
                        for canonical in PLAYER_DISPLAY_ORDER:
                            chosen = choose_preferred_stat([r for r in plist if r['id'] == pid and canonical_player_metric(r.get('metric')) == canonical])
            ...
            st.info('Nenhum dado individual persistido para esta partida. Use o botão de atualização acima.')

        if m.get('status') != 'SCHEDULED':
            if st.button('🧩 Enriquecer esta partida', key=f'e_{selected}', use_container_width=True):
                with st.spinner('Enriquecendo com fontes disponíveis...'):
                    n = enrich([m])
                st.success(f'Enriquecimento concluído: {n} registros.')
                st.rerun()

st.divider()
with st.expander('🩺 Diagnóstico do sistema'):
    u = usage_today('API-Football')
    st.caption(f'API-Football: {u.get("calls", 0)} chamadas hoje • proteção 80/dia e 8/min • cache ativo.')

...
with st.sidebar:
    ...
    optional_selected=st.multiselect('Adicionar ao monitoramento',optional_labels,default=[],key='optional_competitions',placeholder='Selecione outros campeonatos')
    ...
    if st.button('🔄 Atualizar jogos',use_container_width=True,type='primary'):
        ...
        with st.spinner(f'Coletando jogos dos {days} dias selecionados...'):rows=collect(date_from.isoformat(),date_to.isoformat(),competitions=selected_competitions)
        st.session_state['last_collection_count']=len(rows);st.session_state['last_collection_window']=(date_from.isoformat(),date_to.isoformat());st.success(f'{len(rows)} partidas encontradas.')
...
st.caption('Arquitetura: fontes → normalização → identidade canônica → banco → cache → histórico → enriquecimento → análise. A análise consome somente a base persistente.')
...
if st.session_state.get('last_collection_count') is not None:st.caption(f'Última coleta: {st.session_state["last_collection_count"]} partidas persistidas no período selecionado.')
if not window_matches:st.info('Nenhuma partida encontrada na janela atual. Clique em “Atualizar jogos” para coletar os dados das competições selecionadas.')
...
if selected:
    ...
    if not m:st.session_state.pop('selected',None)
    else:
        ...
        if stats or m.get('home_score') is not None or m.get('away_score') is not None:
            ...
            for canonical in MATCH_DISPLAY_ORDER:
                if canonical=='goals':home=m.get('home_score');away=m.get('away_score')
                    ...
                    home_rows=[r for r in stats if r.get('team_id')==m['home_id'] and canonical_match_metric(r.get('metric'))==canonical];away_rows=[r for r in stats if r.get('team_id')==m['away_id'] and canonical_match_metric(r.get('metric'))==canonical]
                    home_row=choose_preferred_stat(home_rows);away_row=choose_preferred_stat(away_rows);home=home_row.get('value') if home_row else None;away=away_row.get('value') if away_row else None
            ...
            st.dataframe(pd.DataFrame(rows),hide_index=True,use_container_width=True);st.caption('Cada indicador aparece uma única vez. Quando existem várias fontes para a mesma métrica, o sistema escolhe uma fonte prioritária e não soma fontes diferentes.')
        else:st.info('Sem estatísticas persistidas para esta partida. O sistema não inventa valores.')

        # HISTÓRICO: preparação de dados é uma operação separada do motor de análise.
        ...
        if len(home_hist)>=3 and len(away_hist)>=3:
            c1,c2=st.columns(2);c1.metric(f'{m["home_name"]} • jogos anteriores',len(home_hist));c2.metric(f'{m["away_name"]} • jogos anteriores',len(away_hist))
            with st.expander(f'⚽ Últimos {len(home_hist)} jogos — {m["home_name"]}',expanded=False):
                st.dataframe(pd.DataFrame([{'Data':manaos_time(x.get('start_time')),'Competição':canonical_competition_label(x.get('competition')),'Jogo':f'{x["home_name"]} {x.get("home_score","-")} × {x.get("away_score","-")} {x["away_name"]}'} for x in home_hist]),hide_index=True,use_container_width=True)
            with st.expander(f'⚽ Últimos {len(away_hist)} jogos — {m["away_name"]}',expanded=False):
                st.dataframe(pd.DataFrame([{'Data':manaos_time(x.get('start_time')),'Competição':canonical_competition_label(x.get('competition')),'Jogo':f'{x["home_name"]} {x.get("home_score","-")} × {x.get("away_score","-")} {x["away_name"]}'} for x in away_hist]),hide_index=True,use_container_width=True)
        else:
            st.warning('Histórico insuficiente para esta partida. O motor não deve transformar ausência de histórico em 0%.')
            if st.button('📚 Construir histórico desta partida',key=f'h_{selected}',use_container_width=True):
                with st.spinner('Coletando histórico das duas equipes e preparando dados individuais...'):result=build_history_for_match(m)
                st.success(f'Histórico preparado: {result["home_matches"]} jogos da casa, {result["away_matches"]} jogos do visitante e {result["player_matches_enriched"]} partidas com dados individuais enriquecidos.');st.rerun()

        if m['status']=='SCHEDULED':
            analysis=build_pre_match_analysis(m);p=analysis['probabilities'];st.subheader('🔮 Análise pré-jogo')
            if p['home'] is None or p['draw'] is None or p['away'] is None:
                st.info('Ainda não há histórico suficiente para calcular probabilidades. Construa o histórico acima; a análise continuará usando somente dados persistidos.')
            else:
            ...
            # Resumo individual histórico, quando já houver dados persistidos.
        ...
        players=get_players(selected);unique_players={x['id']:x for x in players};st.subheader(f'👥 Jogadores • {len(unique_players)}')
        if players:
            teams={}
                ...
                for pid in sorted({p['id'] for p in plist},key=lambda x: next((p['name'] for p in plist if p['id']==x),'')):
                    ...
                    with st.expander(f'👤 {p0["name"]}  •  {position_label(p0.get("position"))}',expanded=False):
                        ...
                        for canonical in PLAYER_DISPLAY_ORDER:
                            candidates=[r for r in pstats if canonical_player_metric(r.get('metric'))==canonical];chosen=choose_preferred_stat(candidates);value=chosen.get('value') if chosen else None;table.append({'Indicador':PLAYER_DISPLAY_LABELS[canonical],'Valor':format_metric_value(canonical,value)})

    ...
    if m:
        ...
        if m['status']=='SCHEDULED':
            if st.button('📚 Atualizar histórico + estatísticas + jogadores',key=f'h_{selected}',use_container_width=True,type='primary'):
                ...
                st.success(f'Histórico: {result["home_matches"]} + {result["away_matches"]} jogos • {result["stats_records"]} estatísticas • {result["player_records"]} registros individuais.');st.rerun()
...
with st.expander('⚙️ Cache'):
    st.caption('As respostas das fontes são cacheadas. A análise não dispara novas chamadas de API; somente a coleta/enriquecimento atualiza a base.')

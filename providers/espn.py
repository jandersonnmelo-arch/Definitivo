...
class ESPNProvider(FootballProvider):
    ...
    def _team_stat_value(cls, stat):
        """Return the numeric team value from ESPN's several stat shapes.

        Some ESPN soccer feeds expose passing as a completed/attempted ratio
        such as ``448/520``.  For the canonical team metric
        ``passes_completed`` the completed side (448) is the value required by
        the analytics engine.  Plain numeric fields continue to be handled by
        _num().
        """
    ...
    def _stat_pairs(cls, group, athlete):
        ...
        keys = (
            group.get('keys') or group.get('labels') or group.get('names') or group.get('descriptions')
            or athlete.get('keys') or athlete.get('labels') or athlete.get('names') or athlete.get('descriptions') or []
        )
            ...
            try:
                ...
                if isinstance(game, dict):
                    ...
                    if not alt_players or not alt_stats:
                        ...
                        alt_players = alt_players or rec_players; alt_stats = alt_stats or rec_stats

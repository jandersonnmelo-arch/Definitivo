...
class ESPNProvider(FootballProvider):
    ...
    def _team_stat_value(cls,stat):
        ...
        for key in ('value','numericValue','total'):
            if key in stat:
                ...
                if n is not None:return n
        raw=stat.get('displayValue');n=cls._num(raw)
        if n is not None:return n
    ...
    def _player_metric(cls,label):
        key=cls._key(label)
        aliases={'goal':'goals','goals_scored':'goals','total_goals':'goals','assist':'assists','accurate_passes':'passes_completed','accuratepasses':'passes_completed','passes_accurate':'passes_completed','total_passes_completed':'passes_completed','total_shots':'shots','shots_total':'shots','shots_on_goal':'shots_on_target','shots_on_target_total':'shots_on_target','shots_woodwork':'shots_woodwork','shots_on_target':'shots_on_target','shots_off_target':'shotsofftarget','total_tackles':'tackles','effective_tackles':'tackles','fouls_committed':'fouls','fouled':'was_fouled','fouls_suffered':'was_fouled','fouls_suffered_total':'was_fouled','minutes':'minutes_played','minutes_played_total':'minutes_played','minutes_played':'minutes_played','yellow_cards':'yellow_cards','red_cards':'red_cards','yellowcards':'yellow_cards','redcards':'red_cards','saves':'saves','keeper_saves':'saves','goalkeeper_saves':'saves','woodwork':'shots_woodwork','offsides':'offsides','tackles':'tackles','passes':'passes','total_passes':'passes','appearances':'appearances'}
    ...
    def _stat_pairs(cls,group,athlete):
        ...
        for i,value in enumerate(values):
            ...
            if isinstance(value,dict):
                label=value.get('key') or value.get('name') or value.get('label') or value.get('displayName') or value.get('description') or label
        ...
        for team_block in box.get('players') or []:
            ...
            for group in groups:
                ...
                for item in athletes:
                    ...
                    ath=item.get('athlete') or item.get('player') or item;pid=ath.get('id') or item.get('id') or item.get('playerId')
                    if not pid:continue
                    try:pid=int(pid)
    ...
    def _extract_players_recursive(cls,node,team_id=None,team_name=None,depth=0):
        ...
        if isinstance(node,list):
            for item in node:
                p,s=cls._extract_players_recursive(item,team_id,team_name,depth+1);players.extend(p);stats.extend(s)
            return cls._dedupe(players,stats)
        if not isinstance(node,dict):return [],[]
        local_team=node.get('team') if isinstance(node.get('team'),dict) else {};tid=node.get('teamId') or local_team.get('id') or team_id;tname=node.get('teamName') or local_team.get('displayName') or local_team.get('name') or team_name;ath=node.get('athlete') or node.get('player')
        if isinstance(ath,dict):
            pid=ath.get('id') or node.get('id') or node.get('playerId');name=ath.get('displayName') or ath.get('fullName') or ath.get('shortName') or node.get('displayName') or node.get('fullName')
            if pid and name:
                try:pid=int(pid)
                ...
                if pid:
                    ...
                    if not pairs and ath is not node:pairs=cls._stat_pairs(node,ath)
                    for label,value in pairs:
                        metric=cls._player_metric(label)
                        if metric!='__player_presence__':stats.append({'player_id':pid,'metric':metric,'value':value,'source':cls.name})
        for key,value in node.items():
            if key in {'plays','leaders','notes','odds'}:continue
            if isinstance(value,(dict,list)):
                p,s=cls._extract_players_recursive(value,tid,tname,depth+1);players.extend(p);stats.extend(s)
        return cls._dedupe(players,stats)
    ...
    def _dedupe(players,stats):
        ...
        for p in players:
            ...
            if key not in p_seen:p_out.append(p);p_seen.add(key)
        s_out=[];s_seen=set()
        for s in stats:
            key=(s.get('player_id'),s.get('metric'),s.get('value'))
            if key not in s_seen:s_out.append(s);s_seen.add(key)
        return p_out,s_out

    @classmethod
    def _debug_vinicius(cls,node,match_id,path='root',depth=0):
        """Read-only probe. It never changes extraction/persistence."""
        if node is None or depth>12:return None
        if isinstance(node,list):
            for i,item in enumerate(node):
                found=cls._debug_vinicius(item,match_id,f'{path}[{i}]',depth+1)
                if found:return found
            return None
        if not isinstance(node,dict):return None
        candidates=[]
        for objname,obj in [('node',node),('athlete',node.get('athlete')),('player',node.get('player'))]:
            if isinstance(obj,dict):
                text=' '.join(str(obj.get(k,'')) for k in ('displayName','fullName','shortName','name'))
                if 'vinicius' in cls._key(text) or 'vinícius' in text.lower():
                    candidates.append((objname,obj))
        if candidates:
            objname,obj=candidates[0]
            summary={k:obj.get(k) for k in ('id','displayName','fullName','shortName','position','keys','labels','names','descriptions','statistics','stats','values','totals') if k in obj}
            group_keys=[]
            for k in ('keys','labels','names','descriptions'):
                if isinstance(node.get(k),list):group_keys=node[k];break
            raw_values=None
            for k in ('statistics','stats','values','totals'):
                if k in node:raw_values=node[k];break
            pairs=cls._stat_pairs(node,obj)
            important=[]
            for label,value in pairs:
                lk=cls._key(label)
                if any(x in lk for x in ('pass','tackle','assist','card','yellow','red')):important.append(f'{label}={value} -> {cls._player_metric(label)}')
            msg=f'VINICIUS_DEBUG path={path}; object={objname}; fields={list(summary.keys())}; ids={summary.get("id")}; name={summary.get("displayName") or summary.get("fullName")}; position={summary.get("position")}; group_keys={group_keys[:40]}; raw_values_type={type(raw_values).__name__}; important_pairs={important[:40]}'
            add_diagnostic('diagnostico_vinicius','OK',msg,cls.name,str(match_id))
            return True
        for key,value in node.items():
            if key in {'plays','leaders','notes','odds'}:continue
            if isinstance(value,(dict,list)):
                found=cls._debug_vinicius(value,match_id,f'{path}.{key}',depth+1)
        ...
        if not players or not player_stats:
            try:
                ...
                if isinstance(game,dict):
                    alt_box=game.get('boxscore') or {};alt_players,alt_stats=self._extract_players(alt_box)
                    if not alt_players or not alt_stats:rec_players,rec_stats=self._extract_players_recursive(game);alt_players=alt_players or rec_players;alt_stats=alt_stats or rec_stats
                    if alt_players:players=alt_players
                    if alt_stats:player_stats=alt_stats
                    self._debug_vinicius(game,match_id)
            except Exception:pass
        self._debug_vinicius(data,match_id)
        if not players or not player_stats:
            try:

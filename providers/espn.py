...
class ESPNProvider(FootballProvider):
    ...
    def _player_metric(cls, label):
        ...
        aliases = {
            ...
            'yellowcards':'yellow_cards','redcards':'red_cards','saves':'saves','keeper_saves':'saves','goalkeeper_saves':'saves',
            'woodwork':'shots_woodwork','offsides':'offsides','tackles':'tackles','passes':'passes',
    ...
    def _extract_players_recursive(cls,node,team_id=None,team_name=None,depth=0):
        ...
        local_team=node.get('team') if isinstance(node.get('team'),dict) else {};tid=node.get('teamId') or local_team.get('id') or team_id;tname=node.get('teamName') or local_team.get('displayName') or local_team.get('name') or team_name

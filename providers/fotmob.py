...
class FotMobProvider(FootballProvider):
    ...
    def _norm_name(value):
        s=unicodedata.normalize('NFKD',str(value or '')).encode('ascii','ignore').decode().lower();s=re.sub(r'\b(fc|cf|sc|ec|ac|club|clube|football|futbol|calcio)\b',' ',s);s=re.sub(r'\b(real)\b',' ',s);s=re.sub(r'\b(de|da|do|dos|das)\b',' ',s);return re.sub(r'[^a-z0-9]+',' ',s).strip()
    @staticmethod
    ...
    def _find_match_id(self,match):
        ...
        terms=[f"{match.get('home_name','')} {match.get('away_name','')}",f"{match.get('away_name','')} {match.get('home_name','')}",str(match.get('home_name',''))];candidates=[];seen=set()
        for term in terms:
            data=get_json(SEARCH,{'term':term,'hits':50,'lang':'en'},HEADERS,provider='FotMob');groups=(data.get('matchSuggest') or data.get('matches') or []) if isinstance(data,dict) else []
    ...
    def _player_from_obj(obj,team_id=None,team_name=None):
        ...
        tid=p.get('teamId') or obj.get('teamId') or team_id;tname=p.get('teamName') or obj.get('teamName') or team_name
        ...
        if isinstance(position,dict):position=position.get('displayName') or position.get('abbreviation')
    ...
    def _extract_lineup_players(cls,lineup,default_team_id=None,default_team_name=None):
        ...
        def add(obj,tid=None,tname=None):
            ...
            if p and p['id'] not in seen:seen.add(p['id']);out.append(p)
...
def _same_names(a,b):
    ...
    def tokens(value):return {x for x in str(value).split() if x not in {'real','de','da','do','dos','das'}}
    ta,tb=tokens(a),tokens(b);common=ta & tb
    if not ta or not tb:return False
    if len(common)>=2:return len(common)>=min(len(ta),len(tb))
